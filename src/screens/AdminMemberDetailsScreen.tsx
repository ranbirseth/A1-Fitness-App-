...
export function AdminMemberDetailsScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<AppStackParamList>>();
  const { memberId } = useRoute<RouteProp<AppStackParamList, 'AdminMemberDetails'>>().params;
  const { user } = useAuth();
  ...
  const loadData = useCallback(
    async (mode: 'initial' | 'refresh') => {
      ...
      setError(null);
      try {
        const [mem, branches] = await Promise.all([getMember(memberId), getBranches()]);
        setMember(mem);
  ...
  const branchLabel =
    member?.branchCode &&
    (adminBranch && adminBranch.branchCode === member.branchCode
      ? adminBranch.name
      : member.branchCode);
