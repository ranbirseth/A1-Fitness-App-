...
export function AdminPaymentsScreen() {
  ...
  const fetchData = useCallback(
    async (mode: LoadingMode) => {
      ...
      try {
        ...
        console.log('[PAYMENT] --> requesting payments', { branchCode, searchTerm });
    ...
    [activePeriod, branchCode, searchTerm]
  );
  ...
  }, [loading, page, total, activePeriod, branchCode, searchTerm]);
  ...
  useEffect(() => {
    fetchData('refresh');
  }, [activePeriod, searchTerm, fetchData]);
  ...
  const handleSendReminders = async () => {
    Alert.alert(
      'Send Reminders',
      `Send renewal reminders to members whose plans expire soon in ${branchCode}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Send',
