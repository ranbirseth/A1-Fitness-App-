...
export function AdminMembersScreen() {
  ...
  const [adminBranch, setAdminBranch] = useState<BranchItem | null>(null);
  ...
  const [filterSheet, setFilterSheet] = useState<'status' | null>(null);
  ...
  const load = useCallback(
    async (mode: 'initial' | 'refresh') => {
      ...
      try {
        ...
        setAdminBranch(branches.find((b) => b.branchCode === branchCode) ?? null);
    ...
    [searchTerm, statusFilter, branchCode]
  );
  ...
  const openDetails = useCallback(
    (member: MemberItem) => {
      navigation.navigate('AdminMemberDetails', { memberId: member._id });
    },
    [navigation]
  );
  ...
  const filteredHasQuery = searchTerm.length > 0 || statusFilter !== 'all';

  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      <SuperadminHeader
        title="Members"
        subtitle="Manage members in your branch."
        right={
